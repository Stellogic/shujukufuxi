...
扩展关系代数：
  ...
  📚 关系代数
  ├── 选择 σ
  │   └── 按条件筛选元组，例如 σ Sno='S1'(SPJ)
  ├── 投影 π
  │   └── 选出需要的属性，例如 π Sno(SPJ)
  ├── 连接 ⋈
  │   └── 把两个关系按公共属性合并，例如 SPJ ⋈ Parts 按 Pno 连接
  ├── 分组聚集 γ
  │   ├── 不分组：γ COUNT(Sno)->num(...)
  │   ├── 按属性分组：γ Pno; SUM(Quantity)->total(...)
  │   └── 常见函数：COUNT、SUM、AVG、MAX、MIN
  └── 派生属性
      └── 例如 TotalWeight = Quantity * Weight

**γ 聚集函数(属性)->结果属性名 (输入关系)**

**γ 分组属性; 聚集函数(属性)->结果属性名 (输入关系)**
...
就是在在spj中出现过的，所以要去重
  ...
  γ COUNT(Sno)->SupplierCount (π Sno(SPJ))

(2) 求所有零件的平均重量。

  γ AVG(Weight)->AvgWeight (Parts)

(3) 求供应商S1提供的每种零件的总数量。

  先筛选供应商 S1 的供应记录，再按零件编号 Pno 分组，对 Quantity
  求和。

  γ Pno; SUM(Quantity)->TotalQuantity (σ Sno='S1'(SPJ))

(4) 求供应商S1供应工程J1的每种零件的总重量。


  这里要注意：
  Quantity 在 SPJ 中，Weight 在 Parts 中，所以必须连接。
    先筛选：

  σ Sno='S1' ∧ Jno='J1'(SPJ)
    再和 Parts 按 Pno 连接，得到每种零件的重量。
  每条供应记录的重量是：
  ...
  γ Pno; SUM(Quantity * Weight)->TotalWeight
    ((σ Sno='S1' ∧ Jno='J1'(SPJ)) ⋈ Parts)
#
