...
参考答案：
(1) π姓名, 性别, 年龄(ρ年龄←当前年份-year(出生年月)(σ籍贯='河南'(S)));

(2) γSUM(学分)→总学分(σ课程性质='必修'(C));
...
(4) π姓名, 课程名, 学分, 成绩(
        ...
        (π学号, 课程号, 成绩(SC))
        ⋈
        (π课程号, 课程名, 学分(σ课程名='数据库原理'(C)))
    );

(5) π学号, 姓名(
        S ⋈ (
            π学号, 课程号(σ成绩>=90(SC)) ÷ π课程号(C)
        )
    );

注：若第(5)题理解为“学生所选课程全部优秀”，也可写为：
π学号, 姓名(S ⋈ (π学号(SC) - π学号(σ成绩<90(SC))))。
